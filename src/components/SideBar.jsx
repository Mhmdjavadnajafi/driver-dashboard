...
export function SideBar({ isOpen }) {
    const menuItems = [
        { icon: <BiCategory />, text: "ایجاد کاربر راننده", path: "/" },
        { icon: <BiUser />, text: "مدیریت رانندگان", path: "/drivers" },
    ];
      ...
      bg-white border-r border-gray-300 shadow-[4px_0_8px_rgba(0,0,0,0.1)]`}
        >
            <div className="font-bold text-center mb-4 pt-3 pb-2">
                <img className="h-15 w-auto mx-auto my-3" src={Logo} alt="logo" />
            </div>
            ...
            <ul className="w-full h-full flex flex-col items-center mt-4 overflow-y-auto">
                ...
                ${isActive
                                    ? "bg-[#B9A278] text-white"
                                    : "bg-[#F5F5F5] text-[#B9A278] hover:bg-[#e0d8b0]"
                                }`
                            }
                        >
                            <div className="text-[20px]">{item.icon}</div>
                            <span
                                className={`transition-all duration-300 ease-in-out overflow-hidden font-bold text-[12px] ${isOpen ? "opacity-100 ml-2" : "opacity-0 w-0"
                                    }`}
                            >
                                {item.text}
                            </span>
