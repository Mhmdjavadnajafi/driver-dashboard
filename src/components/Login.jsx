...
export function Login({ onLogin }) {
    ...
    const handleBlurPhone = () => setError(!/^09\d{9}$/.test(phone.trim()));
    const handleBlurPassword = () => setErrorPassword(password.trim() === "");
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await axios.post("https://api.tda24.ir/api/users_aslii/login/", {
            ...
            if (res.data.success) {
                ...
                setServerError(res.data.message || "نام کاربری یا رمز عبور نادرست است");
            }
        } catch (err) {
            console.error(err);
    ...
    return (
        <div className="w-full vazir-medium  h-screen bg-white relative flex flex-col items-center justify-center">
            ...
            <form className="flex flex-col items-center gap-4" onSubmit={handleSubmit}>
                <div className="vazir-light text-[#909090] font-bold text-[14px]">
                ...
                <input
                    ...
                    value={phone}
                    onChange={(e) => { setPhone(e.target.value); if (/^09\d{0,9}$/.test(e.target.value)) setError(false); }}
                ...
                <input
                    ...
                    type="password"
                    value={password}
                    onChange={(e) => { setPassword(e.target.value); if (e.target.value.trim() !== "") setErrorPassword(false); }}
